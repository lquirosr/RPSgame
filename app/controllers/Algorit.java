...
//Solucion al problema B
	...
	//Metodo que implementa el punto 2 del API
	...
	String win=first.substring(first.indexOf("=")+1,first.length());
	...
	public static void juego_api1(String count){
		String Container = "[";
		List<Person> people = Person.find("order by lastName desc").fetch(Integer.parseInt(count.indexOf("=")+1,count.length()));//el opcional
		for(Person p : people) {
